...
class Model: 
    ...
    def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
        ...
        with tf.control_dependencies(self.update_ops):
            self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learningRate).minimize(self.loss)
    ...
    def setupCNN(self):
        ...
        for i in range(numLayers):
            ...
            pool = tf.nn.max_pool2d(input=learelu, ksize=(1, poolVals[i][0], poolVals[i][1], 1), strides=(1, strideVals[i][0], strideVals[i][1], 1), padding='VALID') 
    ...
    def setupRNN(self):
        ...
        # Collapse layer to remove dimension 32 x 1 x 256 --> 32 x 256 on axis=2
        rnnIn3d = tf.squeeze(self.cnnOut4d, axis=[2])

        # 2 layers of cells which is used to build RNN
        numHidden = 256
